...
class AdvancedCalculatorGUI:
    ...
    def setup_styles(self):
        ...
        self.style.map(
            "Number.TButton",
            background=[
                ("active", "#4a6082"),  # Lighter blue-gray on hover
                ("pressed", "#2c3e50"),
            ],
        )  # Darker when pressed
        ...
        self.style.map(
            "Operator.TButton",
            background=[
                ("active", "#ec7063"),  # Lighter red on hover
                ("pressed", "#c0392b"),
            ],
        )  # Darker when pressed
        ...
        self.style.map(
            "Function.TButton",
            background=[
                ("active", "#5dade2"),  # Lighter blue on hover
                ("pressed", "#2980b9"),
            ],
        )  # Darker when pressed
        ...
        self.style.map(
            "Equal.TButton",
            background=[
                ("active", "#52c882"),  # Lighter green on hover
                ("pressed", "#229954"),
            ],
        )  # Darker when pressed
    ...
    def create_mode_selector(self, parent):
        ...
        mode_label = tk.Label(
            mode_frame, text="Angle Mode:", font=("Arial", 10), bg="#2c3e50", fg="white"
        )
        mode_label.pack(side=tk.LEFT)

        degrees_radio = tk.Radiobutton(
            mode_frame,
            text="Degrees",
    ...
    def create_button_grid(self, parent):
        ...
        buttons = [
            ...
            [
                ("sin", "Function.TButton"),
                ("cos", "Function.TButton"),
                ("tan", "Function.TButton"),
                ("log", "Function.TButton"),
                ("ln", "Function.TButton"),
            ],
            # Row 2: More functions
            [
                ("asin", "Function.TButton"),
                ("acos", "Function.TButton"),
                ("atan", "Function.TButton"),
                ("√", "Function.TButton"),
                ("x²", "Function.TButton"),
            ],
    ...
    def insert_function(self, function_name):
        ...
        if current == "0":
            ...
            new_text = (
                current[: self.cursor_pos] + function_text + current[self.cursor_pos :]
            )
            self.display_var.set(new_text)
            self.cursor_pos += len(function_name) + 1  # Position inside parentheses
    ...
    def evaluate_expression(self, expression):
        ...
        if self.degrees_mode.get():
            ...
            expression = re.sub(
                r"(?<!math\.)\basin\(([^)]+)\)",
                r"math.degrees(math.asin(\1))",
                expression,
            )
            expression = re.sub(
                r"(?<!math\.)\bacos\(([^)]+)\)",
                r"math.degrees(math.acos(\1))",
                expression,
            )
            expression = re.sub(
                r"(?<!math\.)\batan\(([^)]+)\)",
            ...
            expression = re.sub(
                r"(?<!math\.)\bsin\(([^)]+)\)", r"math.sin(\1)", expression
            )
            expression = re.sub(
                r"(?<!math\.)\bcos\(([^)]+)\)", r"math.cos(\1)", expression
            )
            expression = re.sub(
                r"(?<!math\.)\btan\(([^)]+)\)", r"math.tan(\1)", expression
            )
            expression = re.sub(
                r"(?<!math\.)\basin\(([^)]+)\)", r"math.asin(\1)", expression
            )
            expression = re.sub(
                r"(?<!math\.)\bacos\(([^)]+)\)", r"math.acos(\1)", expression
            )
            expression = re.sub(
                r"(?<!math\.)\batan\(([^)]+)\)", r"math.atan(\1)", expression
            )

        # Handle other functions (order matters - specific patterns first)
        expression = re.sub(r"√\(([^)]+)\)", r"math.sqrt(\1)", expression)
        expression = re.sub(
            r"(?<!math\.)\bsqrt\(([^)]+)\)", r"math.sqrt(\1)", expression
        )  # Negative lookbehind to avoid double replacement
        expression = re.sub(r"\bln\(([^)]+)\)", r"math.log(\1)", expression)
        expression = re.sub(
            r"(?<!math\.)\blog\(([^)]+)\)", r"math.log10(\1)", expression
        )  # Negative lookbehind
        expression = re.sub(r"\|([^|]+)\|", r"abs(\1)", expression)  # |x| notation
        expression = re.sub(
            r"(?<!math\.)\babs\(([^)]+)\)", r"abs(\1)", expression
        )  # Negative lookbehind
        expression = re.sub(
            r"\bfact\(([^)]+)\)", r"math.factorial(int(\1))", expression
        )
...
def main():
    ...
    try:
        ...
        messagebox.showerror(
            "Calculator Error", f"Failed to start calculator: {str(e)}"
        )
        root.destroy()
